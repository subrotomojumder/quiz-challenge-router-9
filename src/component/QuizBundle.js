...
const QuizBundle = ({ bundle }) => {
    ...
    return (
        <div className='bg-lime-100 shadow-xl border-2 p-2 rounded-lg lg:max-h-[80vh] lg:flex items-end'>
            <img src={logo} className='lg:max-h-72' alt="" />
            <div className='ml-8 mt-4'>
                <h3>Total-Question {total}</h3>
                <h2 className='text-3xl font-serif font-bold mb-12'>Topic:- {name} </h2>
                <button
                    onClick={()=> handleQuizStart(id)}
                    className=' bg-blue-500 text-white py-2 rounded-lg px-4 hover:bg-red-300 flex items-center font-bold'
                >
                    <p>Quick Start</p>
